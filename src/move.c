...
{
	dbref player_ref = FD_PLAYER(fd), thing_ref, cont_ref;
	char *what = argv[1];
	char *obj = argv[2];
...
do_drop(int fd, int argc, char *argv[])
{
	...
	if (thing.type == TYPE_GARBAGE) {
		nd_writef(player_ref, "You can't drop that.\n");
		return;
	}
...
do_recycle(int fd, int argc, char *argv[])
{
	...
	if(player_ref == NOTHING && thing.owner == GOD) {
		nd_writef(player_ref, "Only God may reclaim God's property.\n");
		return;
	}
	...
	if (!controls(player_ref, thing_ref)) {
		...
		case TYPE_ROOM:
			if (thing.owner != player.owner) {
				nd_writef(player_ref, "Permission denied. (You don't control the room you want to recycle)\n");
				return;
			}
			if (thing_ref == PLAYER_START) {
				nd_writef(player_ref, "That is the player start room, and may not be recycled.\n");
				return;
			}
			if (thing_ref == GLOBAL_ENVIRONMENT) {
				nd_writef(player_ref, "If you want to do that, why don't you just delete the database instead?  Room #0 contains everything, and is needed for database sanity.\n");
				return;
			}
			break;
		case TYPE_PLANT:
		...
		case TYPE_SEAT:
			if (thing.owner != player.owner) {
				nd_writef(player_ref, "Permission denied. (You can't recycle a thing you don't control)\n");
				return;
			}
			break;
		case TYPE_ENTITY:
			...
			break;
		case TYPE_GARBAGE:
			nd_writef(player_ref, "That's already garbage!\n");
			return;
		}
