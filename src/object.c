...
objects_init()
{
	...
	dbref ref;
	FOR_ALL(ref)
		contents_put(obj_get(ref).location, ref);
...
static inline void
object_update(dbref what_ref, double dt) {
	if (obj_get(what_ref).type == TYPE_ENTITY)
		entity_update(what_ref, dt);
}
...
object_move(dbref what_ref, dbref where_ref)
{
	...
	contents_put(what.location, what_ref);
